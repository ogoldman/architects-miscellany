...
Applications often contain elements that run somewhat independently. For example, user interface elements typically run on user-owned devices but connect with services. If such a service is used only or primarily by other elements of the application, then the service itself can be considered part of the application.
